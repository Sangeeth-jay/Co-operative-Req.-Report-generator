...
Public Class Form3
    ...
    Private Sub PrintDocument2_PrintPage(sender As Object, e As PrintPageEventArgs) Handles PrintDocument2.PrintPage
        ...
        txtHeight = PrintDocument2.DefaultPageSettings.PaperSize.Height - PrintDocument2.DefaultPageSettings.Margins.Top - PrintDocument2.DefaultPageSettings.Margins.Bottom - (e.PageBounds.Height - 500)
        ...
        e.Graphics.DrawString(sname + "- රික්විසිෂන් පත", secondaryfont, Brushes.Black, (Width / 2) - 250, topmargin + 25)
        ...
        For intCounter = intStart To numRow
            ...
            e.Graphics.DrawString(DataGridView1.Rows(intCounter).Cells(0).Value, fntText, Brushes.Black, leftmargin + 35, x2 + 4.5)
            e.Graphics.DrawString(DataGridView1.Rows(intCounter).Cells(1).Value, fntText, Brushes.Black, leftmargin + 180, x2 + 4.5)
            e.Graphics.DrawString(DataGridView1.Rows(intCounter).Cells(2).Value, fntText, Brushes.Black, leftmargin + 310, x2 + 4.5)
            e.Graphics.DrawString(DataGridView1.Rows(intCounter).Cells(3).Value, fntText, Brushes.Black, leftmargin + 740, x2 + 4.5)
            e.Graphics.DrawLine(Pens.Black, leftmargin, x2 + 1, leftmargin + 900, x2 + 1)
            intLineNumber += 1
            x2 += 30
        ...
        e.Graphics.DrawString("ආයතන මුද්‍රාව : ", secfont, Brushes.Black, leftmargin + 740, e.PageBounds.Height - 45)
    ...
    End Sub
    Private Sub sesocietyname()

        Dim conn As New OleDbConnection
        conn.ConnectionString = ("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=|DataDirectory|\coop.accdb")
        conn.Open()

        sidn = cmbSocietyName.SelectedIndex.ToString() + 1

        Dim strsql As String
        strsql = "SELECT * FROM Society WHERE Society_id=" + sidn + ";"
        Dim cmd As New OleDbCommand(strsql, conn)
        Dim myreader As OleDbDataReader
        myreader = cmd.ExecuteReader
        myreader.Read()

        sname = myreader("Society_name")
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        strsql = "Insert INTO Amount VALUES ('" & cmbSocietyName.SelectedIndex + 1 & "' ,'" & sname & "', '" & lblTot.Text & "' , '" & lblDate.Text & "');"

        Dim cmd1 As OleDbCommand = New OleDbCommand(strsql, myconnection)

        cmd1.Parameters.Add(New OleDbParameter("Society_id", CType(cmbSocietyName.SelectedIndex.ToString() + 1, String)))
        cmd1.Parameters.Add(New OleDbParameter("Society_name", CType(sname, String)))
        cmd1.Parameters.Add(New OleDbParameter("Full_amount", CType(lblTot.Text, String)))
        cmd1.Parameters.Add(New OleDbParameter("Print_date", CType(lblDate.Text, String)))
